...
for model_name, dataset_name in model_data_list:
    ...
    with torch.no_grad():
        ...
        dataset = DisentDataset(dataset=data,factors=c, sampler=SingleSampler(), transform=preprocess)
        model = model.to(device)
        get_repr = lambda x: rep_func(x)
        metrics = {
            **metric_dci(dataset, get_repr, num_train=60, num_test=9, show_progress=True),
